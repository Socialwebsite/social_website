<!-- full div-->
<div id='background_wall'>

		<div id="Accordion1" class="Accordion" tabindex="0">  <!-- tab panel -->
		  <div class="AccordionPanel">
			<div class="AccordionPanelTab">edu</div>
			<div class="AccordionPanelContent">Content 1</div>
		  </div>
		  <div class="AccordionPanel">
			<div class="AccordionPanelTab">work</div>
			<div class="AccordionPanelContent">Content 2</div>
		  </div>
		  <div class="AccordionPanel">
			<div class="AccordionPanelTab">fav</div>
			<div class="AccordionPanelContent">favvvvvvvvvvvvvvvvvvvvv</div>
		  </div>
		</div>

				<div id="user_wall">
					<div id="update_status">
						<form action='#' method='POST' id='status_form'>
						<input type='field' placeholder='Whats on your mind ?' id='status_field' class='field' name='status_field' /> 
						<button type='submit' id='status_button'  class='button' name='status_button'>Update Status</button>
						<a href='#' id='update_image_music'  class='button'> Add Images/Music </a>
						
						
						</form>
						
					
					</div>
						<div id="update_link">
						</div>
							<div id="comment">
							</div>

				</div>
						<div id="adds">
						
						</div>


</div>
